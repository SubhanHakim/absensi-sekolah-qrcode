            ...
            <p class="text-gray-500 mb-6">
                ...
                @elseif($periode == 'mingguan')
                    Minggu: <span class="font-semibold">
                        {{ \Carbon\Carbon::parse($tanggal)->startOfWeek()->format('d-m-Y') }} s/d
                        ...
                        @foreach ($students as $student)
                            <tr class="hover:bg-blue-50 even:bg-gray-50 transition">
                                ...
                                <td class="py-2 px-4 border-b border-gray-200">
                                    @if ($periode == 'harian')
                                        @if (isset($absensi[$student->id]) && $absensi[$student->id]->where('tanggal', $tanggal)->count())
                                            <span
                                                ...
                                                Sudah Absen
                                                ({{ ucfirst($absensi[$student->id]->where('tanggal', $tanggal)->first()->status) }})
                                            </span>
                                        @else
                                            <span
                                    ...
                                    @else
                                        @php
                                            $count = isset($absensi[$student->id])
                                                ? $absensi[$student->id]->count()
                                                : 0;
                                            $hadirDates = isset($absensi[$student->id])
                                                ? $absensi[$student->id]
                                                    ->pluck('tanggal')
                                        ...
                                        @endphp
                                        <span
                                            class="inline-flex items-center gap-1 px-2 py-1 rounded bg-blue-100 text-blue-700 font-semibold text-xs"
                                            title="Tanggal hadir: {{ $hadirDates }}">
