    ...
    <div class="flex flex-col items-center justify-center min-h-[60vh] bg-gradient-to-br from-blue-50 to-blue-100 py-8">
        <div class="w-full max-w-2xl bg-white rounded-xl shadow-lg border border-blue-200 p-8">
            <form method="GET" class="mb-6 flex flex-wrap gap-2 items-center">
                ...
                <input type="month" id="bulan" name="bulan" value="{{ request('bulan', now()->format('Y-m')) }}"
            ...
            <div class="overflow-x-auto rounded-lg">
                <table class="min-w-full text-sm border border-gray-200 rounded-lg shadow">
                    ...
                    <tbody>
                            ...
                            <tr class="hover:bg-blue-50 even:bg-gray-50 transition">
                                ...
                                <td class="py-2 px-4 border-b border-gray-200">
                                    @if ($absen->status == 'hadir')
                                        <span
                                            class="inline-flex items-center gap-1 px-2 py-1 rounded bg-green-100 text-green-700 font-semibold text-xs">
                                            <i class="ti ti-check text-base"></i> Hadir
                                        </span>
                                    @elseif($absen->status == 'izin')
                                        <span
                                            class="inline-flex items-center gap-1 px-2 py-1 rounded bg-yellow-100 text-yellow-700 font-semibold text-xs">
                                            <i class="ti ti-alert-circle text-base"></i> Izin
                                        </span>
                                    @elseif($absen->status == 'sakit')
                                        <span
                                            class="inline-flex items-center gap-1 px-2 py-1 rounded bg-orange-100 text-orange-700 font-semibold text-xs">
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="text-center py-4 text-gray-500">Belum ada data absensi bulan
                                    ini.</td>
                            </tr>
                        @endforelse
                    </tbody>
