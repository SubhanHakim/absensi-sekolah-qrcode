...
class AbsensiController extends Controller
{
    ...
    public function processAbsen(Request $request)
    {
        ...
        if ($student) {
            ...
            if ($sudahAbsen) {
                return response()->json([
                    'success' => false,
                    'message' => 'Siswa ' . $student->nama . ' sudah absen hari ini.',
            ...
            $now = now();
            $batasTelat = $now->copy()->setTime(7, 0, 0); // Batas telat jam 07:00
            ...
            try {
                ...
                // Kirim notifikasi WA dalam background
                $this->sendWhatsAppNotification($student, $status);

                return response()->json([
                    'success' => true,
                    'message' => 'Absensi berhasil untuk ' . $student->nama,
                    'student_name' => $student->nama,
                    'status' => $status,
        ...
        return response()->json([
            'success' => false,
            'message' => 'Siswa tidak ditemukan! Pastikan QR Code valid.',
            'qr_code' => $kode
        ]);
    }

    /**
     * Method terpisah untuk mengirim notifikasi WA
     */
    private function sendWhatsAppNotification($student, $status)
    {
        ...
        if ($parent && $parent->no_hp) {
            ...
            $pesan = "Anak Anda, {$student->nama}, telah melakukan absensi dan statusnya: {$statusText} pada " . now()->format('d-m-Y H:i') . ".";

            try {
                $response = \Illuminate\Support\Facades\Http::timeout(10)->withHeaders([
                    'Authorization' => env('Xmxc4Jw9NAnZQDjpHo1o
                    ')
